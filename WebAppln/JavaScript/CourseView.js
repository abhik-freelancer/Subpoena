...
(function($) {
    ...
    $.fn.drag_drop_selectable.moveBetweenLists = function(item_id, old_list_id, new_list_id) {
        ...
        //now move DOM Object.
        var fromContainer = $('ul[dds=' + old_list_id + ']').attr('id');

        if (fromContainer == "AvailableInstructor") {
            //alert('hi');
            $('ul[dds=' + old_list_id + ']').find('li[dds=' + item_id + ']').removeClass($.fn.drag_drop_selectable.settings[old_list_id].ghostClass).clone().appendTo($('ul[dds=' + new_list_id + ']'));
        } else {
            //alert('hi2');
...
$(function() {
    mychange = function($list) {
        ...
        if (fromContainer != null && toContainer != null) {
            if (toContainer != fromContainer) {
                ...
                insIds = String($.dds.serialize($list.attr('id'))).split(',');
                if (toContainer != "AvailableInstructor") {
                    if (insIds.length > 1) {
                        // jQuery Popup Dialog  
                        var txt = 'Do you want to change this Instructor?<input type="hidden" id="toContainerid" name="toContainerid" value="' + toContainer + '" />';

                        $.prompt(txt, {
                            buttons: { Ok: true, Cancel: false },
                            callback: function(v, m, f) {

                                var tocon = f.toContainerid;

                                if (v) {
                                    $("#" + tocon + " li:first").remove();

                                    var instructorIds = new Array();
                                    instructorIds = String($.dds.serialize($list.attr('id'))).split(',');
                                    PageMethods.StoreInstructor("AvailableInstructor", tocon, instructorIds.pop(), Ondone, OnFailure);

                                }
                                else {
                                    $("#" + tocon + " li:last").remove();
                                }
                            }
                        });
                    } else {
                        PageMethods.StoreInstructor(fromContainer, toContainer, insIds.pop(), Ondone, OnFailure);
                    }
                } else {
                    $("#" + toContainer + " li:last").remove();
                    PageMethods.StoreInstructor(fromContainer, toContainer, insIds.pop(), Ondone, OnFailure);
                }
    ...
    function AssignNewAUs(result) {
        ...
        for (var x = 0; x <= len; x++) {
            var item = null;
            item = values[x].split(',');
            UpDatePane(item[0], item[1]);
        }
    }
    function UpDatePane(category, au) {

    }

    $('#list_1_serialised').html($.dds.serialize('list_1'));
    $('#list_2_serialised').html($.dds.serialize('list_2'));
    $('#list_Mapped_Sections').html($.dds.serialize('list_3'));
});

// courseFinder when course Click

//    function courseFinderClick(courseId) {
//        document.getElementById('<% = HdnFldCourse.ClientID %>').value = courseId;
//        __doPostBack('', '');
//    }

//    // Course Finder Show Hide other detail
//    function showHideCourseDtl(showhide) {
//        var ele1 = document.getElementById("ssd");
//        if (showhide == "display") {
