<%@ Page Language="C#" AutoEventWireup="true" EnableViewState="true" MasterPageFile="../Site.master" CodeBehind="UserRegList.aspx.cs" Inherits="Website.Pages.UserRegList" %>

<asp:Content ID="Content22" ContentPlaceHolderID="MainContent" runat="server">
    <div class="row paddingbottom15">
                    <div class="col-xs-12 col-sm-6 col-md-6">
                        <h3 class="font-md">User List</h3>
                    </div>
                    <div class="col-xs-12 col-sm-6 col-md-6">
                        <a class="btn btn-xs btn-primary pull-right" href="UserRegistration.aspx" onclick="" >New Registration</a>
                    </div>
                </div>
 <div class="preview">
...
<script type="text/javascript">
    ...
    jQuery("#jqgrid").jqGrid({
        ...
        datatype: 'json',
        height: '360',
        colNames: ['Actions', 'Id', 'First Name', 'Last Name', 'Email', 'Group Name', 'Role'],
        colModel: [
			{ name: 'act', index: 'act', sortable: false, search: false, width: 70 },
             { name: 'UserId', label: 'UserId', hidden: true },
			{ name: 'UserFirstName', index: 'UserFirstName', sortable: true },
			{ name: 'UserLastName', index: 'UserLastName', sortable: true },
		    { name: 'UserEmail', index: 'UserEmail', sortable: true },
            { name: 'GroupName', index: 'GroupName', sortable: true },
            { name: 'userRole', index: 'userRole', sortable: true }
        ],
    ...
    function editRow(row_ID) {
        var sitename = jQuery("#jqgrid").getCell(row_ID, 'UserId');
        //location.href = 'Modify?USR_ID=' + sitename;
        //alert(sitename);
        window.location.href = 'UserRegistration.aspx?EditId=' + sitename;
        //window.open();
    ...
    function deleteRow(row_ID) {
        if (confirm("Do you Want to Delete this record ? ")) {
            var sitename = jQuery("#jqgrid").getCell(row_ID, 'UserId');
            // e.preventDefault();
            var url = "UserRegList.aspx?Type=deleted&&DeleteId=" + sitename;
